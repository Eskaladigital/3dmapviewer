...
const CATEGORIES = [
  ...
  { id: 'restaurant', label: 'Restaurante', icon: <UtensilsCrossed size={16} /> },
...
const getFurnitureIcon = (type: string, category: string, size = 24) => {
  ...
  if (t.includes('restaurant') || t.includes('pizza_oven') || t.includes('buffet') || t.includes('salad_bar') || t.includes('host_stand') || t.includes('food_display') || t.includes('coffee_machine_pro') || t.includes('wine_rack') || t.includes('wine_cooler') || t.includes('commercial_') || t.includes('deep_fryer') || t.includes('plancha_grill') || t.includes('outdoor_') || t.includes('industrial_kitchen') || t.includes('soda_fountain') || t.includes('ice_machine') || t.includes('service_station')) return <UtensilsCrossed size={size} strokeWidth={1.5} />

  // Fallbacks por categoría
  if (category === 'restaurant') return <UtensilsCrossed size={size} strokeWidth={1.5} />
